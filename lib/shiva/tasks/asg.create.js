...
const InvalidArgumentError = require('../../common/errors/invalid-argument-error')
...
function shivaASGProvision (job) {
  return Promise
    .try(function validateJob () {
      ...
      if (job.isPersonalAccount !== false) {
    ...
    .then(() => {
      return publiser.publishTask('asg.policy.scale-out.create', {
        githubId: job.githubId
      })
    })
    .catch(InvalidArgumentError, err => {
      throw WorkerStopError('Invalid Argument', { err })
    })
}
