...
module.exports = class DockPool {
  ...
  static _getInstancesFromASG (asgInfo) {
    ...
    if (!instances && instances.length === 0) {
      throw new WorkerError('no instances available in dock pool')
    }
    return instances
  }
  ...
  static _findValidInstanceIdFromInstances (instances) {
    ...
    if (!outInstance) {
      throw new WorkerError('no healthy instance available in dock pool')
    }
   ...
   * @resolves {undefined}
   */
  static detachInstanceFromPool (instanceId) {
