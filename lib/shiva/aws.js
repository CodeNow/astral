 ...
 * @param {object} params Specific parameters to provide to the aws
 *   runInstances call.
 * @return {Promise} A promise that resolves when all ec2 instances have
 *   started.
 */
AWSProvider.prototype.createInstances = function (cluster, params) {
  params = params || {};

  params.UserData = new Buffer(
 ...
 */
AWSProvider.prototype.getUserDataScript = function () {
  return Mustache.render(this.userDataTemplate, {
    consul_hostname: process.env.CONSUL_HOSTNAME
  });
};
...
/**
 * Promisified version of the ec2 `describeInstances` method.
 * @param {object} params Paramters for the instance describe method.
 * @param {array} param.InstanceIds Ids of the instances to describe.
 * @return {Promise} A promise for the describeInstances method.
 */
AWSProvider.prototype.describeInstances = function (params) {
  var self = this;
  return new Promise(function (resolve, reject) {
    self.ec2.describeInstances(params, function (err, data) {
      if (err) { reject(err); }
      resolve(data);
    });
  });
...
AWSProvider.prototype.getDefaultInstanceParams = function () {
  return {
    ...
    SecurityGroupIds: process.env.AWS_DOCK_SECURITY_GROUPS.split(',').map(trim),
    BlockDeviceMappings: [],
