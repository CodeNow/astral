...
Model.prototype.count = function (column) {
  return this.db.count(column).table(this.table);
};
 ...
 * model.update({ami_id: 'ami-afe302'}, data, function (err) {
 *   // Err will be populated if you made a mistake in your data...
 * });
 *
 * @param {object|string} query object or id (Primary key id) for the row to update.
 * @param {object} data Data to update for the record in the database.
 * @return {knex~promise} A promise for the update query.
 */
Model.prototype.update = function (queryOrId, data) {
  data.updated_at = this.db.raw('now()');
  var query = isObject(queryOrId) ? queryOrId : this._wherePrimaryEq(queryOrId);
  return this.db(this.table).where(query).update(data);
};
